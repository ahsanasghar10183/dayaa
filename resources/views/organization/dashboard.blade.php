<x-organization-sidebar-layout>
    <x-slot name="header">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('admin.dashboard.welcome_back', ['name' => auth()->user()->name]) }}</h1>
        ...
        <form method="GET" action="{{ route('organization.dashboard') }}" class="flex flex-wrap items-end gap-3">
            ...
            <div>
                ...
                <select name="period"
                    ...
                    <option value="last_6_months" {{ request('period') == 'last_6_months' ? 'selected' : '' }}>{{ __('admin.dashboard.last_6_months') }}</option>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 hover:shadow-lg transition-shadow">
            ...
            <div>
                <p class="text-sm text-gray-600 font-medium">{{ __('admin.dashboard.total_campaigns') }}</p>
                <p class="text-3xl font-bold text-gray-900 mt-1">{{ $stats['total_campaigns'] }}</p>
                <p class="mt-3 text-sm text-green-600 font-medium">{{ $stats['active_campaigns'] }} {{ __('admin.dashboard.active') }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 hover:shadow-lg transition-shadow">
            ...
            <div>
                <p class="text-sm text-gray-600 font-medium">{{ __('admin.dashboard.total_devices') }}</p>
                <p class="text-3xl font-bold text-gray-900 mt-1">{{ $stats['total_devices'] }}</p>
                <p class="mt-3 text-sm text-green-600 font-medium">{{ $stats['online_devices'] }} {{ __('admin.dashboard.online') }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 hover:shadow-lg transition-shadow">
            ...
            <div>
                <p class="text-sm text-gray-600 font-medium">{{ __('admin.dashboard.current_month_revenue') }}</p>
                <p class="text-3xl font-bold text-gray-900 mt-1">€{{ number_format($stats['this_month_amount'], 2) }}</p>
                <p class="mt-3 text-sm text-gray-600">{{ $stats['this_month_donations'] }} {{ __('admin.dashboard.donations') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div class="mb-6">
                ...
                <p class="text-sm text-gray-600 mt-1">
                    @if(request('period') == 'custom')
                        {{ \Carbon\Carbon::parse(request('start_date'))->format('M d, Y') }} - {{ \Carbon\Carbon::parse(request('end_date'))->format('M d, Y') }}
                    @else
                        {{ ucwords(str_replace('_', ' ', request('period', 'This month'))) }} {{ __('admin.dashboard.activity') }}
                    @endif
                </p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 hover:shadow-md transition-shadow">
            <div class="mb-6">
                <h3 class="text-lg font-semibold text-gray-900">{{ __('admin.dashboard.campaign_performance') }}</h3>
                <p class="text-sm text-gray-600 mt-1">{{ __('admin.dashboard.top_performing') }}</p>
            </div>
            <div style="height: 300px;">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                        ...
                        <a href="{{ route('organization.donations.index') }}" class="text-sm text-primary-600 hover:text-primary-700 font-medium">{{ __('admin.dashboard.view_all') }} →</a>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('admin.dashboard.method') }}</th>
                    ...
                    <div class="px-6 py-12 text-center">
                        ...
                        <p class="mt-4 text-sm text-gray-600">{{ __('admin.dashboard.no_donations_yet') }}</p>
                        <p class="text-xs text-gray-500">{{ __('admin.dashboard.no_donations_text') }}</p>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold text-gray-900">{{ __('admin.dashboard.active_campaigns') }}</h3>
                        <a href="{{ route('organization.campaigns.index') }}" class="text-sm text-primary-600 hover:text-primary-700 font-medium">{{ __('admin.dashboard.view_all') }} →</a>
                    </div>
                    ...
                    <div class="text-center py-4">
                        <p class="text-sm text-gray-500">{{ __('admin.dashboard.no_campaigns_active') }}</p>
                        <a href="{{ route('organization.campaigns.create') }}" class="mt-2 inline-block btn-primary text-xs">{{ __('admin.dashboard.create_campaign') }}</a>
                    </div>
                    @endif
                </div>
                ...
                <div class="bg-gradient-dayaa rounded-2xl shadow-lg p-6 text-white">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="border-t border-white border-opacity-20 pt-4">
                            <p class="text-sm opacity-90">{{ __('admin.dashboard.avg_donation') }}</p>
                ...
                @if($subscription)
                <div class="bg-white rounded-2xl shadow-sm p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('admin.dashboard.subscription_info') }}</h3>
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-sm font-semibold">€{{ number_format($subscription->price, 2) }}{{ __('admin.dashboard.per_month') }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                        ...
                        <div class="pt-3 border-t border-gray-200">
                            <p class="text-xs text-gray-500">{{ __('admin.dashboard.next_billing') }}: {{ $subscription->next_billing_date->format('M d, Y') }}</p>
                        </div>
                        @endif
                    </div>
                </div>
