<x-super-admin-sidebar-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="font-semibold text-xl sm:text-2xl text-gray-800">{{ __('admin.shop.create_new_product') }}</h2>
            <a href="{{ route('super-admin.shop.products.index') }}" class="inline-flex items-center justify-center px-4 py-2 bg-gray-200 border border-transparent rounded-lg font-semibold text-gray-700 hover:bg-gray-300 transition">
        ...
        <form action="{{ route('super-admin.shop.products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="space-y-6">
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 hover:border-blue-500 transition" :class="productType === 'simple' ? 'border-blue-500 ring-2 ring-blue-500' : ''">
                            ...
                            <div class="flex flex-1">
                                <div class="flex flex-col">
                                    <span class="block text-sm font-semibold text-gray-900">{{ __('admin.shop.simple_product') }}</span>
                                    <span class="mt-1 flex items-center text-xs text-gray-500">{{ __('admin.shop.simple_product_desc') }}</span>
                                </div>
                            </div>
                        ...
                        <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 hover:border-blue-500 transition" :class="productType === 'variable' ? 'border-blue-500 ring-2 ring-blue-500' : ''">
                            ...
                            <div class="flex flex-1">
                                <div class="flex flex-col">
                                    <span class="block text-sm font-semibold text-gray-900">{{ __('admin.shop.variable_product') }}</span>
                                    <span class="mt-1 flex items-center text-xs text-gray-500">{{ __('admin.shop.variable_product_desc') }}</span>
                                </div>
                            </div>
                ...
                <div x-show="productType === 'simple'" x-transition class="border-t pt-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('admin.shop.product_details') }}</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- SKU -->
                        <div>
                            <label for="simple_sku" class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.shop.sku_required') }}</label>
                        ...
                        <!-- Price -->
                        <div>
                            <label for="simple_price" class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.shop.price_eur_required') }}</label>
                        ...
                        <!-- Compare Price -->
                        <div>
                            <label for="simple_compare_price" class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.shop.compare_price_eur') }}</label>
                        ...
                        <!-- Quantity -->
                        <div>
                            <label for="simple_quantity" class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.shop.stock_quantity_required') }}</label>
                    ...
                    <div class="mt-6" x-data="imageUpload()">
                        ...
                        <p class="mt-2 text-xs text-gray-500">{{ __('admin.shop.first_image_primary_hint') }}</p>
                    </div>
                </div>
                ...
                <div x-show="productType === 'variable'" x-transition class="border-t pt-6">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900">{{ __('admin.shop.product_variations') }}</h3>
                            <p class="text-sm text-gray-500 mt-1">{{ __('admin.shop.add_variations_hint') }}</p>
                        </div>
                    ...
                    <div class="space-y-4">
                        <template x-for="(variation, index) in variations" :key="index">
                            <div class="border border-gray-200 rounded-lg p-6 bg-gray-50" x-data="variationImageUpload(index)">
                                <div class="flex items-start justify-between mb-4">
                                    <h4 class="text-md font-semibold text-gray-900" x-text="'{{ __('admin.shop.variation_label') }} ' + (index + 1)"></h4>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <!-- Variation Name -->
                                    <div class="md:col-span-2">
                                        <label :for="'variation_name_' + index" class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.shop.variation_name_required') }}</label>
                                        ...
                                        <p class="mt-1 text-xs text-gray-500">{{ __('admin.shop.variation_name_hint') }}</p>
                                    </div>

                                    <!-- SKU -->
                                    <div>
                                        <label :for="'variation_sku_' + index" class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.shop.sku_required') }}</label>
                                    ...
                                    <!-- Price -->
                                    <div>
                                        <label :for="'variation_price_' + index" class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.shop.price_eur_required') }}</label>
                                    ...
                                    <!-- Compare Price -->
                                    <div>
                                        <label :for="'variation_compare_price_' + index" class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.shop.compare_price_eur') }}</label>
                                    ...
                                    <!-- Quantity -->
                                    <div>
                                        <label :for="'variation_quantity_' + index" class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.shop.stock_quantity_required') }}</label>
                                    ...
                                    <!-- Variation Images -->
                                    <div class="md:col-span-2">
                                        <label class="block text-sm font-medium text-gray-700 mb-3">{{ __('admin.shop.variation_images') }}</label>
                                        ...
                                        <div class="relative">
                                            ...
                                            <label :for="'variation_images_' + index"
                                                ...
                                                <div class="flex flex-col items-center justify-center py-4">
                                                    ...
                                                    <p class="text-xs text-gray-500 group-hover:text-gray-700"><span class="font-semibold">{{ __('admin.shop.click_to_upload') }}</span> images</p>
                                                    <p class="text-xs text-gray-500">{{ __('admin.shop.variation_image_upload_hint') }}</p>
                                                </div>
                                            </label>
                                        </div>

                                        <p class="mt-2 text-xs text-gray-500">{{ __('admin.shop.first_image_primary_variation_hint') }}</p>
                                    </div>
                                    ...
                                    <div class="md:col-span-2">
                                        <div class="flex items-center">
                                            ...
                                            <label :for="'variation_active_' + index" class="ml-2 text-sm text-gray-700">{{ __('admin.shop.active_available_purchase') }}</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div x-show="variations.length === 0" class="text-center py-12 border-2 border-dashed border-gray-300 rounded-lg">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('admin.shop.no_variations_added') }}</h3>
                            <p class="mt-1 text-sm text-gray-500">{{ __('admin.shop.get_started_first_variation') }}</p>
                            <div class="mt-6">
                                <button type="button" @click="addVariation" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                ...
                <div class="border-t pt-6 space-y-2">
                    <h3 class="text-lg font-semibold text-gray-900 mb-3">{{ __('admin.shop.product_status') }}</h3>
                    <div class="flex items-center">
    ...
    <script>
        const shopI18n = {
            maxImages: @json(__('admin.shop.max_images_alert', ['max' => 10])),
            maxImagesVariation: @json(__('admin.shop.max_images_variation_alert', ['max' => 10])),
            fileTooLarge: @json(__('admin.shop.file_too_large_alert', ['file' => '__FILE__'])),
            notImage: @json(__('admin.shop.not_image_alert', ['file' => '__FILE__'])),
        };
            ...
            return {
                ...
                handleFileSelect(event) {
                    ...
                    if (this.images.length + files.length > this.maxImages) {
                        alert(shopI18n.maxImages);
                        return;
                    }

                    files.forEach(file => {
                        if (file.size > 2 * 1024 * 1024) {
                            alert(shopI18n.fileTooLarge.replace('__FILE__', file.name));
                            return;
                        }

                        if (!file.type.startsWith('image/')) {
                            alert(shopI18n.notImage.replace('__FILE__', file.name));
                            return;
                        }
        ...
        function variationImageUpload(variationIndex) {
            return {
                ...
                handleFileSelect(event) {
                    ...
                    if (this.images.length + files.length > this.maxImages) {
                        alert(shopI18n.maxImagesVariation);
                        return;
                    }
